...
<script type="text/javascript">
    ...
    var pdf_reader = {
        ...
        data_url: '<?php echo build_slug('ajax/ajax_pdf_page/ebooks'); ?>',
        ...
        get_history: function(){
            $.get(pdf_reader.get_history_url,{id: pdf_reader.id}, function(returned){
                if(returned.history != undefined && returned.history != false){
                    ...
                    pdf_reader.get_page(returned.history);
                }else{
                    //pdf_reader.render_page(pdf_reader.cur_page);
                    pdf_reader.get_page(pdf_reader.cur_page);
                }
            });
        },
        ...
        get_data: function(){
            return $.post(this.data_url, {
                data_id: this.id,
                page: this.cur_page
            }, function(returned){
                pdf_reader.pdfData = atob(returned.pdf);
                pdf_reader.pages = returned.pages;
                var $input = $('#page_num');
                var max = $input.attr('max');
                if(max != pdf_reader.pages){
                    $input.attr('max', pdf_reader.pages);
                }
            }).promise();
        },
        get_vars: function(){
        ...
        init: function(){
            ...
            this.get_history(); //Will get page as part of history check.
        },
        get_page: function(page_num){
            ...
            this.get_data().done(function(){
                pdf_reader.lib.getDocument({
                    data: pdf_reader.pdfData
                }).promise.then(function(pdfDoc_){
                    pdf_reader.pdfDoc = pdfDoc_;
                    pdf_reader.loaded = true;
                    pdf_reader.render_page();
                });
            });
        },
        render_page: function(){
            var page_num = 1; //It should always be page 1
            //document.body.scrollTop = document.documentElement.scrollTop = 0;
            //$('#page_num').val(page_num);
