...
$sql = 'SELECT * FROM `data_meta` WHERE `data_meta_name` = "starring" AND `data_meta_content` LIKE :star';
...
foreach($final_results as $r){
	$string .= '<a class="button" href="'.build_slug('view/'.$r['data_id'], [], 'media').'">'.$r['data_name'].'</a><br><br>';
}
